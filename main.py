...
def run_Scraper(category: str, user_id: str, minimum_followers: Union[int, None] = Query(default=None), language: Union[str, None] = Query(default=None), viewer_count: Union[str, None] = Query(default=None),  maximum_followers: Union[str, None] = Query(default=None)):
    thread = threading.Thread(target=start,kwargs={"c":category, "user_id":user_id, "min_f":minimum_followers if minimum_followers else 0, "choice_l":language if language else ANYT, "min_viewer_c":viewer_count if viewer_count else 0, "max_f": maximum_followers if maximum_followers else 10000000000})
    thread.start()  
